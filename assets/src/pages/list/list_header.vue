<template>
  <div class="list_header">
    <a href="#/home"
       class="btn_index">首页</a>
    <div class="list_category"
         v-on:click="chooseCategory">{{currenSelect.text}}
      <i class="fa fa-angle-down"></i>
    </div>
    <div class="category_box"
         v-bind:class="{slide_down:isShow}">
      <ul>
        <li v-for="(item, index) in categorys"
            :data-id="index"
            :key="index"
            v-on:click="onSelect($event)"
            v-bind:class="[index==currenSelect.id?'active':'']">{{item.text}}</li>
      </ul>
    </div>
    <div class="mask"
         v-show="isShow"
         v-on:click="onMask"></div>
  </div>
</template>
