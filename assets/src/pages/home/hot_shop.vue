<template>
  <div class="db_hot_shop"
       v-infinite-scroll="getHotShops"
       infinite-scroll-disabled="busy"
       infinite-scroll-distance="{num}">
    <!-- 一个店铺 -->
    <div class="db_shop_item"
         v-for="(shop,k) in hotShops"
         :key="k">
      <!-- 店铺名称和简介 -->
      <div class="shop_info">
        <h3 class="shop_top">
          <a href=""
             class="shop_img"><img :src="shop.icon"></a>
          <a href="#"
             class="shop_name">{{shop.name}}</a>
        </h3>
      ...
      <div class="shop_product db_products">
        <ul>
          <li v-for="(pd,k) in shop.products"
              v-bind:key="k">
            <div class="pdt_item">
              <a :href="pd.url+'/'+pd.id"
                 class="pdt_img">
